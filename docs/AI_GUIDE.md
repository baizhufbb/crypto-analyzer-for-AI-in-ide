...
2. **信息增强**：主动进行联网搜索（获取最新宏观/新闻），将外部基本面信息纳入决策。
...
> 你就应该自动完成识别→搜新闻→拉数据→分析→报告的完整流程，不要求用户手动敲命令。
...
2.  **自动倒计时**：
    ...
    - 命令示例：`uv run scripts/timer_reminder.py 15` (设置 15 分钟倒计时)。
    - 若判断不需要持续跟踪，则跳过此步骤。
3.  **循环启动**：
    - 当倒计时脚本运行结束（听到提醒音）后，默认视为下一次对话周期的开始。
    - 你应准备好在用户响应后，重复执行"时间确认 -> 信息增强 -> 数据获取 -> 分析报告"的标准流程.